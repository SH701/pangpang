...
export default function InterestSelector({ selected, onChange }: InterestSelectorProps) {
  ...
  return (
    <div className="space-y-4 flex flex-col gap-20 ">
      <h2 className="text-xl font-semibold mb-6.5 ml-3.5">Please select your <br /> interests</h2>
          ...
          return (
            <button
              ...
              className={`px-3 py-1 rounded-full transition-colors whitespace-nowrap
                ${isActive ? 'bg-sky-100 text-black border' : 'border border-gray-300 text-gray-700 hover:bg-gray-100'}`}
            >
              {content}
            </button>
          );
